...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/[category]/[slug]/[offerSlug]">): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    path: `/${detail.game.categorySlug}/${slug}/${offerSlug}`,
...
export default async function OfferDetailPage({
  params,
}: PageProps<"/[locale]/[category]/[slug]/[offerSlug]">) {
  const locale = await resolveLocaleParam(params);
  const { slug, offerSlug } = await params;
      ...
      {relatedOffers.length > 0 ? (
        <Section spacing="normal">
          <SectionHeader
            ...
            viewAllHref={`/${locale}/${game.categorySlug}/${game.slug}`}
