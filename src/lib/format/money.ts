...
export function formatPrice(amount: number, currency: string, locale: Locale): string {
  ...
  // Arabic reads the amount first, then the symbol; single-character symbols
  // attach tightly so the price reads as one unit (e.g. "يبدأ من 5.09$"),
  // multi-letter codes trail with a space.
  if (locale === "ar") {
    return symbol.length === 1 ? `${formatted}${symbol}` : `${formatted} ${symbol}`;
  }
