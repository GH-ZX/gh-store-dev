...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const [stats, attention, kpis, earnings, series, latest, wallets] = await Promise.all([
    getAdminOverviewStats(), getAttentionCounts(), getSalesKpis(), getEarnings(),
    getDailySeries(14), getLatestOrders(5), getWalletCards(),
  ]);
  return { locale, stats, attention, kpis, earnings, series, latest, wallets };
}

export default function DashboardIndex() {
  const { locale, stats, attention, kpis, earnings, series, latest, wallets } = useLoaderData<typeof loader>();
  ...
  const lifetimeLine = [
    `${messages.overview.stats.games}: ${stats.products ?? "—"}`,
    `${messages.overview.stats.activeGames}: ${stats.activeProducts ?? "—"}`,
