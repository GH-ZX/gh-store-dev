...
import { RechargeMethodsForm } from "@/components/admin/recharge-methods-form";
...
import {
  ...
  getSamStatus,
  getTelegramStatus,
} from "@server/legacy/lib/services/admin-settings.service";
import { getRechargeQueues } from "@server/lib/services/admin-recharge.service";
...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const [status, callback, maxstoreStatus, batstoreStatus, logs, samStatus, binanceStatus, samOverview, axiomStatus, fulfillmentSettings, telegramStatus, igdbStatus, walletCards, rechargeQueues] =
    await Promise.all([
      ...
      getIgdbStatus(),
      getWalletCards(),
      createSupabaseServerClient().then(getRechargeQueues),
    ]);

  return { locale, status, callback, maxstoreStatus, batstoreStatus, logs, samStatus, binanceStatus, samOverview, axiomStatus, fulfillmentSettings, telegramStatus, igdbStatus, walletCards, rechargeQueues };
}

export default function Page() {
 const { locale, status, callback, maxstoreStatus, batstoreStatus, logs, samStatus, binanceStatus, samOverview, axiomStatus, fulfillmentSettings, telegramStatus, igdbStatus, walletCards, rechargeQueues } = useLoaderData<typeof loader>();
