...
export async function saveRechargeMethodsAction(_state: LiveEditState, formData: FormData): Promise<LiveEditState> {
 await requireAdmin();
 const parsed = parseRechargeMethodsInput(formText(formData, "methods") ?? "");
 if (!parsed.ok) return { error: "invalid_input", notice: null };
 try { await saveRechargeSettings(await createSupabaseServerClient(), { methods: parsed.methods }); }
 catch (error) { logFailure("admin.recharge", "recharge_methods_save_failed", error); return { error: "unknown", notice: null }; }
 revalidatePath("/", "layout");
