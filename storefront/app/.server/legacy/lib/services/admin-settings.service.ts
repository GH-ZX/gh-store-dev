...
import { memoizeRequest } from "@server/request-context";
import { getSupabaseEnv } from "@server/lib/supabase/env";

import { requireAdmin } from "@server/lib/auth/guards";
import { resetLogDestination } from "@server/lib/logging/logger";
...
import {
  ...
  type BinanceStatus,
} from "@server/lib/settings/binance-settings";
import {
  ...
  type BatStoreStatus,
} from "@server/lib/settings/batstore-settings";
import { checkCallbackUrl, type CallbackReachability } from "@server/lib/settings/callback-url";
import {
  mergeIgdbSettings,
  readIgdbCredentials,
  toIgdbStatus,
  ...
  type IgdbStatus,
} from "@server/legacy/lib/settings/igdb-settings";
import {
  ...
  type MaxStoreStatus,
} from "@server/lib/settings/maxstore-settings";
import { newCallbackSecret } from "@server/legacy/lib/settings/callback-secret";
import {
  mergeG2BulkSettings,
  ...
  type G2BulkStatus,
} from "@server/lib/settings/provider-settings";
import {
  mergeSamSettings,
  readSamCredentials,
  toSamStatus,
...
import { g2bulkCallbackUrl, functionUrl, telegramWebhookFunction } from "@server/lib/supabase/functions-url";
...
import {
  ...
  type TelegramStatus,
} from "@server/legacy/lib/settings/telegram-settings";
import {
  mergeRefundOnFulfillmentFailure,
  readRefundOnFulfillmentFailure,
  type FulfillmentSettings,
} from "@server/lib/settings/fulfillment-settings";
 ...
 */
const readProviders = () => memoizeRequest("admin-provider-settings", readProvidersFresh);
