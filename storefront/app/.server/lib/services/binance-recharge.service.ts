...
import { BinanceClient, BinanceError, isBinancePaid } from "@server/providers/binance/client";
import { toMerchantTradeNo } from "@server/providers/binance/signing";
...
function webhookUrl(): string | null {
  const supabaseUrl = runtimeVar("SUPABASE_URL");
...
export async function getBinancePaymentOptions(): Promise<{ enabled: boolean; currency: string }> {
  if (!hasServiceRoleKey()) return { enabled: false, currency: "USDT" };
...
async function attemptBinanceTopUp(supabase: SupabaseClient<Database>, input: { amount: number; locale?: Locale }): Promise<StartBinanceResult> {
  ...
  const siteUrl = runtimeVar("APP_URL") ?? "";
 ...
 */
export async function getMyBinanceInvoice(supabase: SupabaseClient<Database>, key: string): Promise<BinanceInvoiceView | null> {
  const user = await requireUserId(supabase);
 ...
 */
export async function syncMyBinanceInvoice(supabase: SupabaseClient<Database>, invoiceKey: string): Promise<BinanceSyncResult> {
  const invoice = await getMyBinanceInvoice(supabase, invoiceKey);
