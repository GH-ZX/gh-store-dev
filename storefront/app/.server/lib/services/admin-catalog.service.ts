 ...
 */
export async function updateAdminOffers(
  client: Client,
  isAdmin: boolean,
  gameId: string,
  rows: AdminOfferUpdate[],
): Promise<void> {
 ...
 */
export async function setAdminProductProviderLink(
  client: Client,
  ...
  if (nextUrl !== null && service) {
    await service.from("audit_logs").insert({
      actor_user_id: admin.id,
      action: "catalog.provider_link_saved",
      entity_type: "game",
      entity_id: gameId,
      new_values: { url: nextUrl },
    });
  }
}
...
export async function deleteAdminProduct(
  ...
  if (service) {
    await service.from("audit_logs").insert({
      ...
      entity_type: "game",
      entity_id: data.id,
      new_values: { slug: data.slug, nameAr: data.name_ar, nameEn: data.name_en },
    });
  }
}
...
export async function removeImportedProduct(
  ...
  try {
    await deleteAdminProduct(client, service, admin, mapping.game_id);
  } catch (deleteError) {
    return {
      ok: false,
