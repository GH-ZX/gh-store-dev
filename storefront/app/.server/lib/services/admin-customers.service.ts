...
import { getOrders, type AdminOrderRow } from "@server/lib/services/admin-orders.service";
import { listCustomerRecharges, type AdminRechargeRequest } from "@server/lib/services/admin-recharge.service";

import { refuseActiveChange, refuseRoleChange } from "@server/lib/auth/admin-changes";
...
import type { HeaderWalletCustomer } from "@server/lib/wallet-panel";
import type { WalletTransaction, WalletTransactionType } from "@server/lib/services/wallet.service";
...
export async function listAdminCustomers(supabase: Client, options: { query?: string } = {}): Promise<AdminCustomer[]> {
 ...
 */
export async function listTopWallets(supabase: Client, limit = 25): Promise<HeaderWalletCustomer[]> {
  await requireAdminId(supabase);
...
async function countActiveAdmins(supabase: Client): Promise<number> {
 ...
 * their own tests.
 */
export async function setCustomerRole(supabase: Client, userId: string, nextRole: string): Promise<void> {
  ...
  const refusal = refuseRoleChange({
    ...
    activeAdminCount: await countActiveAdmins(supabase),
  });
...
export async function setCustomerActive(supabase: Client, userId: string, nextActive: boolean): Promise<void> {
  const admin = await requireAdminId(supabase);
...
export async function getAdminCustomer(supabase: Client, userId: string): Promise<AdminCustomerDetail | null> {
  await requireAdminId(supabase);
  ...
  const [{ data, error }, { data: transactions }, orders, recharges] = await Promise.all([
    ...
    supabase
      ...
      .order("created_at", { ascending: false })
      .limit(20),
    getOrders(supabase, { userId, limit: 25 }),
    listCustomerRecharges(supabase, userId, 20),
  ]);
 ...
 */
export async function adjustCustomerBalance(supabase: Client, input: {
  userId: string;
  amount: number;
  description: string;
 ...
 */
export async function sendCustomerMessage(supabase: Client, input: {
  userId: string;
  title: string;
  body: string;
