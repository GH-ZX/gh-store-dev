...
import type { SupportStatus } from "@server/lib/support/status";
...
const THREAD_COLUMNS =
  "id, subject, status, message_count, last_message_at, created_at";
...
export async function openSupportThread(
  ...
  const result = await attemptOpenThread(supabase, input);
...
async function attemptOpenThread(
  supabase: SupabaseClient,
  input: {
    subject: string;
    body: string;
...
export async function replyToThread(
  ...
  const result = await attemptReply(supabase, input, "customer");

  logOutcome("support", "thread_replied", result, {
...
export async function replyAsAdmin(
  ...
  const result = await attemptReply(supabase, input, "admin");
