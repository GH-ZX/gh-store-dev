...
import { notify, notifyAdmins } from "@server/lib/services/notification.service";
import {
  readSamCredentials,
  type SamCredentials,
  type SamMethod,
} from "@server/lib/settings/sam-settings";
import { samCallbackUrl } from "@server/lib/supabase/functions-url";
...
import { SamClient, resolveSamWallet, sypForUsd } from "@server/providers/sam/client";
import { SamError } from "@server/providers/sam/errors";
...
export async function getSamPaymentOptions(supabase: SupabaseClient<Database>): Promise<SamPaymentOptions> {
...
function webhookUrl(secret: string): string {
  return samCallbackUrl(runtimeVar("SUPABASE_URL") ?? "", secret);
}
 ...
 */
export async function startSamTopUp(supabase: SupabaseClient<Database>, input: { amount: number; method: SamMethod }): Promise<StartTopUpResult> {
  const result = await attemptSamTopUp(supabase, input);
...
async function attemptSamTopUp(supabase: SupabaseClient<Database>, input: { amount: number; method: SamMethod }): Promise<StartTopUpResult> {
...
export async function getMySamInvoice(supabase: SupabaseClient<Database>, samInvoiceId: string): Promise<SamInvoiceView | null> {
  const user = await requireUserId(supabase);
 ...
 */
export async function syncSamInvoice(supabase: SupabaseClient<Database>, samInvoiceId: string): Promise<SettleResult> {
  const user = await requireUserId(supabase);
 ...
 */
export async function verifySamPayment(supabase: SupabaseClient<Database>, input: {
  samInvoiceId: string;
