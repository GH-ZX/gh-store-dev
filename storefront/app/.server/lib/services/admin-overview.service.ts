...
import { SamClient } from "@server/providers/sam/client";
 ...
 * rows.
 *
 * So Postgres does the arithmetic and request-scoped memoization shares the result
...
type CatalogCounters = {
  // The RPC's deployed JSON contract retained these keys after its SQL moved
  // to products; accept both versions while exposing product names to the UI.
  products?: number;
...
const readOverviewSnapshot = () => memoizeRequest("overview-snapshot", async (): Promise<OverviewSnapshot | null> => {
...
export async function getAdminOverviewStats(): Promise<AdminOverviewStats> {
  ...
  return {
    products: catalog?.products ?? catalog?.games ?? null,
    activeProducts: catalog?.active_products ?? catalog?.active_games ?? null,
