...
} from "@server/lib/services/invoice-document";
import { getMyOrder, type MyOrderDetail } from "@server/lib/services/orders-read.service";
import { getMyRechargeRequest } from "@server/lib/services/recharge.service";
...
export async function getOrderInvoice(
  ...
  const order = await getMyOrder(session, userId, locale, orderId);
 ...
 */
export async function getRechargeInvoice(
  session: SupabaseClient,
  service: SupabaseClient | null,
  userId: string,
  requestId: string,
): Promise<RechargeInvoice | null> {
  const request = await getMyRechargeRequest(session, userId, requestId);
