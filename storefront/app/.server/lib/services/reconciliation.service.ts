...
import { syncBinanceInvoice } from "@server/lib/services/binance-sync.service";
import { reconcileOrder } from "@server/fulfillment";
import type { ReconcileOutcome } from "@server/fulfillment/types";
 ...
 */
async function expireStaleSamInvoices(supabase: SupabaseClient): Promise<number> {
 ...
 * five-request quota.
 */
async function expireStaleBinanceInvoices(supabase: SupabaseClient): Promise<number> {
  const now = new Date().toISOString();
  const neverExpiresCutoff = new Date(Date.now() - 60 * 60_000).toISOString();
 ...
 */
async function settlePendingBinanceInvoices(supabase: SupabaseClient): Promise<{ checked: number; credited: number }> {
  ...
  for (const invoice of data ?? []) {
    try {
      const result = await syncBinanceInvoice(supabase, invoice.merchant_trade_no);
 ...
 * silent.
 */
async function expireStaleRechargeRequests(supabase: SupabaseClient): Promise<number> {
 ...
 */
async function findStuckOrders(supabase: SupabaseClient, limit: number): Promise<{ id: string; order_number: string }[]> {
 ...
 */
async function recordRun(supabase: SupabaseClient, run: ReconcileRun, startedAt: string): Promise<void> {
...
export async function reconcileStuckOrders(
  ...
  const [samExpired, binanceExpired, binance] = await Promise.all([
    expireStaleSamInvoices(supabase),
    expireStaleBinanceInvoices(supabase),
    settlePendingBinanceInvoices(supabase),
  ]);
  ...
  run.requestsExpired = await expireStaleRechargeRequests(supabase);

  const orders = await findStuckOrders(supabase, limit);
  ...
  if (
    ...
    await recordRun(supabase, run, startedAt);
...
export async function getLastReconcileRun(
  supabase: SupabaseClient | null,
): Promise<LastReconcileRun | null> {
  if (!supabase) {
    return null;
  }
