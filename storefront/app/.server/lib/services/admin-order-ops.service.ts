...
import { isSettledOrderStatus } from "@server/lib/orders/order-status";
 ...
 */
export async function refundOrderManually(supabase: Client, orderId: string, note: string): Promise<void> {
  const admin = await requireAdminId(supabase);
  const order = await loadOrder(orderId);
 ...
 */
export async function markDelivered(supabase: Client, 
  orderId: string,
  note: string,
 ...
 */
export async function resendDeliveryNotification(supabase: Client, orderId: string): Promise<void> {
  const admin = await requireAdminId(supabase);
  const order = await loadOrder(orderId);
