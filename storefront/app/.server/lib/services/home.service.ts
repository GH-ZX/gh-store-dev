...
import {
  ...
  getSaleOffers,
  getTrendingOffers,
} from "@server/lib/services/home-catalog.service";
import { getPublishedReviews, type StoreReview } from "@server/lib/services/public-home-reviews.service";
import { getBinancePaymentOptions } from "@server/lib/services/binance-recharge.service";
import { getSamPaymentOptions } from "@server/lib/services/sam-recharge.service";
  ...
  switch (section.type) {
    case "trust_strip": {
      ...
      const [sam, binance] = await Promise.all([
        safely("trust_strip", () => getSamPaymentOptions(client), {
          enabled: false,
    ...
    case "trending_offers": {
      const offers = await safely(
        section.type,
        () => getTrendingOffers(client, locale, section.limit),
        [],
      );
      return offers.length > 0 ? { kind: "offers", section, offers } : null;
    }
    case "games": {
      const games = await safely(section.type, () => getActiveProducts(client, locale, section.limit), []);
      return games.length > 0 ? { kind: "games", section, games } : null;
    }
    case "category": {
      const games = await safely(
        section.type,
        () => getProductsByCategories(client, locale, section.categoryIds, section.limit),
        [],
      );
      return games.length > 0 ? { kind: "games", section, games } : null;
    }
    case "product_picks": {
      const games = await safely(section.type, () => getProductsByIds(client, locale, section.productIds), []);
      return games.length > 0 ? { kind: "games", section, games } : null;
    }
    case "gift_cards": {
      const offers = await safely(
    ...
    case "sale_offers": {
      const offers = await safely(section.type, () => getSaleOffers(client, locale, section.limit), []);
      return offers.length > 0 ? { kind: "offers", section, offers } : null;
    }
    case "suggested_offers": {
      const offers = await safely(section.type, () => getBestSellers(client, locale, section.limit), []);
      return offers.length > 0 ? { kind: "offers", section, offers } : null;
    }
    case "offer_picks": {
      const offers = await safely(section.type, () => getOffersByIds(client, locale, section.offerIds), []);
      return offers.length > 0 ? { kind: "offers", section, offers } : null;
    }
    case "customer_reviews": {
      const reviews = await safely(
        section.type,
        () => getPublishedReviews(client, locale, section.limit, section.reviewIds),
        [],
      );
...
export async function resolveHomeSections(
  ...
  const resolved = await Promise.all(enabled.map((section) => resolveSection(client, locale, section)));

  return resolved.filter((section): section is ResolvedHomeSection => section !== null);
}

export async function getHomeCarousel(client: SupabaseClient, locale: Locale, layout: HomeSection[]): Promise<HomeCarousel> {
  ...
  return {
    section,
    products: await safely("carousel", () => getCarouselProducts(client, locale, section.limit), []),
  };
}
