...
import { maskSecret } from "@server/lib/settings/provider-settings";
...
export function readBinanceCredentials(providers: unknown): BinanceCredentials {
  ...
  const apiKey = settings?.api_key?.trim() || runtimeVar("BINANCE_PAY_API_KEY") || null;
  const apiSecret = settings?.api_secret?.trim() || runtimeVar("BINANCE_PAY_SECRET_KEY") || null;
