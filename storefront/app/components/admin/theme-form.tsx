...
export function ThemeForm({ theme, messages, errors }: ThemeFormProps) {
  const [state, formAction, pending] = useWebsiteAction("saveThemeAction");
   ...
   * what visitors get after saving. Subsequent edits continue to preview,
   * and the tag removes itself when the form unmounts.
   */
  ...
  useEffect(() => {
    ...
    // Keep previewing subsequent edits even after an earlier successful save.
    tag.textContent = themeStyle(previewTheme);
  });

  return (
    <form method="post" onSubmit={formAction} className="grid gap-4">
        <input type="hidden" name="intent" value="saveThemeAction" />
