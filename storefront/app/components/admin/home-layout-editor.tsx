...
import type { HomePickCandidates } from "@server/lib/services/admin-website.service";
...
export function HomeLayoutEditor({
  ...
  const [saveState, saveAction, saving] = useWebsiteAction("saveHomeLayoutAction");
  const [resetState, resetAction, resetting] = useWebsiteAction("resetHomeLayoutAction");
  ...
  return (
    <div className="grid gap-5">
      <form method="post" onSubmit={saveAction} className="grid gap-4">
        <input type="hidden" name="intent" value="saveHomeLayoutAction" />
      ...
      <div className="grid gap-2 border-t border-[var(--line)] pt-5 sm:flex sm:items-end sm:gap-3">
        ...
        <form method="post" onSubmit={resetAction} className="sm:ms-auto">
        <input type="hidden" name="intent" value="resetHomeLayoutAction" />
