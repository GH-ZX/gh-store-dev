      ...
      for (const card of subset) {
        ...
        try {
          const response = await fetch("/api/admin/wallet-sync", {
            ...
            body: new URLSearchParams({ intent: "syncWallet", key: card.key }),
          });
          if (!response.ok) throw new Error("Wallet sync failed");
          const result = await response.json() as
            | { ok: true; balances: WalletCardView["balances"]; syncedAt: string }
          ...
          return (
            <li key={card.key}>
              {embedded ? (
                ...
                <Link
                  to={`/${locale}/dashboard/providers#${anchorFor(card.provider)}`}
      ...
      <span className="min-w-0 flex-1">
        ...
        {syncedAt ? (
          <span
            ...
            >
            {labels.lastSynced} <bdi dir="ltr">{syncedAt.slice(0, 16).replace("T", " ")}</bdi>
          </span>
        ) : null}
      </span>
    </>
