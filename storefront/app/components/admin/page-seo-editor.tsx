...
function PageRow({
  ...
  const [state, formAction, pending] = useWebsiteAction("savePageSeoAction");
  ...
  return (
    <details className="group rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)]">
      ...
      <form method="post" onSubmit={formAction} className="grid gap-4 border-t border-[var(--line)] p-4">
        <input type="hidden" name="intent" value="savePageSeoAction" />
