...
} from "@/components/commerce/recharge-state";
import type { BinanceInvoiceView } from "@server/lib/services/binance-recharge.service";
...
export function BinancePaymentPanel({ locale, messages, invoice, returnTo }: BinancePaymentPanelProps) {
  ...
  const status = invoice.status === "credited" ? "credited"
    : CLOSED_STATUSES.has(invoice.status) ? invoice.status
    : state.status ?? "pending";
  const settled = status === "credited" || CLOSED_STATUSES.has(status.toLowerCase());
