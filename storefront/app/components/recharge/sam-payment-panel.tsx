...
import {
  ...
  type SamTopUpState,
} from "@/components/commerce/sam-state";
import type { SamInvoiceView } from "@server/lib/services/sam-recharge.service";
...
export function SamPaymentPanel({ locale, messages, invoice }: SamPaymentPanelProps) {
  ...
  const [verifyState, _verifyAction, verifying, verifyActionSubmit] = useCommerceAction<SamTopUpState>('verifySamPaymentAction',
    INITIAL_SAM_TOPUP_STATE,
  );
   ...
   */
  const reportedStatuses = [invoice.status, verifyState.status, checkState.status];
  const status = reportedStatuses.includes("credited") ? "credited"
    : reportedStatuses.includes("awaiting_review") ? "awaiting_review"
    : reportedStatuses.some((value) => ["expired", "failed", "cancelled"].includes(value)) ? "expired"
    : "pending";
  ...
  }, [settled, checking, verifying, invoice.samInvoiceId, locale, checkAction]);
  ...
  return (
    <div className="grid gap-5">
      ...
      <form onSubmit={checkActionSubmit} className="flex flex-wrap items-center gap-3">
        <input type="hidden" name="samInvoiceId" value={invoice.samInvoiceId} />
        <input type="hidden" name="locale" value={locale} />
        <Button type="submit" variant="secondary" disabled={checking || verifying}>
      ...
      <form onSubmit={verifyActionSubmit} className="grid gap-3 border-t border-[var(--line)] pt-5">
        <input type="hidden" name="samInvoiceId" value={invoice.samInvoiceId} />
        <input type="hidden" name="locale" value={locale} />
