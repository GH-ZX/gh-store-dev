...
## Supplier availability follow-up

The storefront now filters products without an active offer from browse, category, carousel, related-product and search results. The direct product page remains reachable and clearly says the item is currently unavailable; inactive offer routes cannot continue into checkout. BatStore import now treats zero or missing stock as unavailable, records the stock snapshot, and restores offers it parked when stock returns. A later admin edit clears that restoration marker. Availability still reflects the last manual provider import. The nine offers found inactive before this work have not been re-enabled or repriced; their live supplier status must be checked in Dashboard → Providers, because earlier imports did not record whether those deactivations were automatic or manual.

Production DOM checks in English and Arabic confirmed the affected products are absent from product lists, the Services category and Proton VPN search, while its direct product page returns the localized unavailable message. No screenshots or images were inspected.
